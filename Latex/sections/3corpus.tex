...
First, I generate a list of the one hundred most frequent mass nouns,
...
Hence, I searched for all sequences N\Sub{1}N\Sub{2} where N\Sub{2} was one of the mass noun lemmas extracted in in the first step.
...
The full set of manually annotated variables for the main study given in Table~\ref{tab:variables}.%
...
Since instances of the \PGCa\ make up for only a minute fraction of all genitives, I still use \textit{Genitives} as a regressor.%
...
As of today, it is not recommended for use in published research, however.
...
All features described in Section~\ref{sec:variablesandannotation} were annotated except for the ones which do not apply in the non-alternating case (\textit{Kindattraction} and \textit{Measureattraction} and \textit{Measurecase}).
...
\footnote{In fact, due to processing considerations with the COW interface at the time, only ten slices of DECOW14A were used, which add up to approximately 10 billion tokens.}
