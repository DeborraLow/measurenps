...
However, none of the constructions (\PGCd, \PGCa, \NACb, and \NACa) are exclusively associated with one of the discussed meanings, and thus I do not see how one could specify schemas, for which ``membership is not a matter of degree'' and whose properties are ``fully compatible with all the members of the category'' \citep[371]{Langacker1987}.
A modelling in terms of a probabilistic similarity effects is more appropriate.
See also the argument in \citealp[70--71]{Taylor2003} and my discussion in Section~\ref{sec:cogocl}.}
...
\subsubsection{Items, exemplar effects, and hierarchical models}
...
It should be noted that such and exemplar-type effect would have to exist in addition to the predicted semantic prototype effect for measure lemmas described in Section~\ref{sec:prototypeeffects}, namely the effect of semantic classes of measure nouns.



To control for the possibility that a potential lemma attraction strength effect is merely an artefact of lemma-specific tendencies, such simple lemma-specific tendencies will have to be accounted for in the modelling as well.
Additionally, 


This leads to a more complex model structure if a generalised linear mixed model (GLMM) is specified, as is customary in alternation modelling (see \citealp{Gries2015} for an argument in favour of such models in corpus linguistic studies; \citealp{GelmanHill2006} (especially part 2) for a comprehensive text book on the subject).
...
In a truly hierarchical model, however, the varying intercept itself comes with an additional linear model where second-level fixed effects predict the group-level effect.
To illustrate, the simple model specification in (\ref{eq:glmm}) is a hierarchical logistic regression model.

\begin{equation}
  Pr(y_i=1)=logit^{-1}(\beta^0_{j[i]}+\beta^1\cdot x^1_i+\beta^2\cdot x^2_i)
  \label{eq:glmm}
\end{equation}

The varying intercept $\beta^0_{j[i]}$ adds a constant term for group $j$ to which $i$ belongs (encoded here as $j[i]$, a notation borrowed from \citealp{GelmanHill2006}).
Each $\beta$ is a fixed-effect coefficient, and each $x$ is an observation-level variable.
(Here, two fixed effects are included for illustration purposes.)
...
This says that the intercepts follow a normal distribution with a standard deviation of $\sigma$ and a mean predicted from the model $\gamma_0+\gamma^1\cdot u_j$, where $\gamma^0$ is the second-level intercept, other $\gamma$s the second-level coefficients, and each $u$ a second-level predictor variable.
If $\beta^0_{j[i]}$ were a lemma random effect and we also included, for example, lemma frequency as a control variable, then $u_j$ could be the lemma frequency predictor which helps to predict the lemma frequency instead of just having simple per-lemma constants.
