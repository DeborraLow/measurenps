...
% \SetWatermarkColor[rgb]{1,0.2,0.2}
% \SetWatermarkText{\textsf{Submitted April 2017.}}
...
\begin{document}
  ...
  \abstract{In this paper, a case alternation occurring in German measure noun phrases is examined from the perspective of cognitively oriented corpus linguistics.
    In pseudo-partitive constructions such as \textit{eine Tasse guter Kaffe} `a cup of good coffee', the embedded kind-denoting noun either agrees in its case with the measure head noun or it is a genitive as in \textit{eine Tasse guten Kaffees}.
    ...
    I assume that the alternants are modelled after non-alternating neighbouring constructions, which represent the prototypes more purely.
    Primarily, it is argued that the frequencies with which individual lemmas occur in the prototypical constructions partially predict which alternant is chosen in the alternating case.
    Another major prediction is that the genitive construction is prototypical of more strongly grammaticalised measure nouns.
    I present a large-scale corpus study using the DECOW web corpus which supports the theoretical analysis.
    In the statistical analysis of the corpus study, it is discussed whether Bayesian estimators lead to substantially different results compared to Likelihood Maximisation methods in studies like this one.
    The model is estimated with Maximum Likelihood and Bayesian methods for comparison.
    Finally, a forced choice experiment and a self-paced reading experiment are reported.
    The usage-based findings from the corpus study show a clear correlation with the behaviour of cognitive agents in both experiments.
    The present study therefore contributes to a now well-established area of research into alternations using both corpus and experimental methods.}
...
It would be highly surprising if this were possible, and it is the same for experimental methods.
...
I call the alternants with an adjective but no determiner in the kind noun phrase \NACa, and \PGCa, respectively.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:starkschwach:a}\gll mit ein\textbf{-em} stark\textbf{-en} Kaffe\\
                                      with a strong coffee\\}
    \ex{\label{ex:starkschwach:b}\gll mit stark\textbf{-em} Kaffe\\
                                      with strong coffee\\}
  \end{xlist}
\end{exe}
...
Second, I now focus on grammaticalisation.
...
Fourth and finally, it was found that the \PGCa\ is more typical of higher registers or even exclusive to written language (see \citealp[320--323]{Hentschel1993}).
...
Thus, newspaper text or any other text that conforms strongly to normative grammars might not represent the alternation phenomenon fully (and without bias) because authors and proof-readers might favour one alternative or the other explicitly because the have to follow normative guidelines.
...
I applied a three-stage process in order to obtain such a sample.
It consisted of three steps:

\begin{enumerate}[i.]
  \item\label{enum:data:step1} generating a list of the one hundred most frequent mass nouns,
  \item\label{enum:data:step2} deriving a list of all measure nouns with which the mass nouns co-occur in the \NACb 
...
\footnote{
  ...
  Three main points speak against such an approach in the present case (but see \citealp[246--249]{Levshina2016} for a more comprehensive approach albeit for a different purpose).
  First, the goal here is to quantify how often lemmas occur in the \PGCd\ and the \NACb, and these construction do not compete at all but are rather mutually exclusive.
  Collostructional approaches are not made such scenarios.
  ...
  Third, I tried using collexeme strength as a regressor, and the results were unsatisfactory compared to the simple quotient used here.
}
...
H% owever, most similar studies (dating back to \citealp{BresnanEa2007}) use an estimator (MLE) that is not frequentist in a narrow sense, and which is as cognitively plausible as Bayesian estimators because it most likely produces the same results in these studies.

If in Levshina's study, results from Bayesian and MLE methods did, in fact, \textit{not} converge, it would have been an ideal occasion to demonstrate the superiority of the algorithms used in Bayesian estimation.
...
Also, the probabilities that the corpus-based model assigns to the two alternants in these sentences is used as the main regressor in both studies.
...
I present a forced-choice experiment in this Section and not a split-100 experiment, mainly because in a dry-run of the experiment, participants complained about the unnaturalness of the task of distributing a probability mass across two alternants.
