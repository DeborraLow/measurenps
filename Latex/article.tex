...
This often called a \textit{validation} of the corpus-derived findings, but \citet[303]{Divjak2016} criticises this choice of words \textit{because it creates the impression that behavioral experimental data is inherently more valuable than textual data}, citing \cite{TummersEa2005}, who state that a corpus is \textit{a sample of spontaneous language use that is} (\textit{generally}) \textit{realized by native speakers}.
...
As a theoretical framework, I assume a model of similarity-based classification in the sense of Prototype Theory.
...
I am sceptical whether corpus analysis alone could ever decide which theory is more adequate, given that substantial doubt has been voiced whether even experimental methods are ultimately able to do so \citep{Barsalou1990}.%
...
All studies mentioned at the outset of Section~\ref{sec:cogocl} use GLM(M)s \citep{BresnanEa2007,BresnanHay2010,BresnanFord2010,DivjakArppe2013,Gries2015,NessetJanda2010}.
Over the past few years, however, modified or alternative methods have been proposed.
From among these methods, I just make a few remarks on Bayesian estimation (see \citealp{GelmanEa2014}), as it was proposed in \cite{Levshina2016,Divjak2016a}.
...
Bayesian approaches (in the now commonly found interpretation), on the other hand, condition on the particular data and quantify inductively the probability of model parameters given the data.
...
Yet, neither frequentist nor Bayesian methods were conceived as cognitive models, but as systems of inference for researchers (see above, and see also \citealp[302]{Divjak2016a}).
...
MLE, like inductive Bayesianism, conditions on the data in as much as it searches for the most likely set of parameters given the data.
...
However, most similar studies (back to \citealp{BresnanEa2007}) use an estimator (MLE) that is not frequentist, and most likely as cognitively plausible as Bayesian estimators.
If for Levshina's study, results did, in fact, \textit{not} converge, it would have been an ideal occasion to demonstrate the superiority of the algorithms used in Bayesian estimation.
...
In essence, participants distribute a probability mass between two variants, which is intended to produce more subtle results compared to what is essentially a two-alternative forced-choice task \cite{Rosenbach2013}.
The split-100 paradigm has been criticised in \cite{ArppeJaervikivi2007}, reiterated in \cite{DivjakEa2016}.
...
I present a forced-choice experiment in Section~\ref{sec:exp:fc} and not a split-100 experiment, mainly because in a dry-run of the experiment, participants complained about the unnaturalness of the task of assigning a probability mass to two variants.
...
As was pointed out in Section \ref{sec:cogocl}, there is a strong interest in combining corpus-based findings with experiments.
Therefore, this section and the next present the results of two experiments wherein I compare results from the corpus study with experimental results.
...
None of them had ever participated in any kind of reading experiment.
