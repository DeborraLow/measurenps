...
Notice that the kind noun is obligatorily in the genitive and that case agreement with the measure noun is not an option (but see immediately below).
...
In step (\ref{enum:data:step3}), each of these 2,365 noun–noun combinations was queried in the target constructions (PGC and NAC with just an adjective) individually in each of the first ten slices of DECOW (roughly 10 billion tokens).
...
\footnote{In a similar fashion, the 100 most frequent measure nouns occurring with plural kind nouns were bootstrapped and queried.
...
As stated in Section~\ref{sec:germanmeasurenps}, the NAC is virtually never used with plural kind nouns.
