...
Put differently, the \NACa\ is a \NACb in disguise, and the \PGCa\ is a \PGCd in disguise.
In a probabilistic framework, the crucial question is what controls speakers' decisions to use a a \NACb in disguise or a \PGCd in disguise.
If the \PGCd\ instantiates the PGC prototype and the \NACb\ the NAC prototype, the choice between \PGCa\ and \NACa\ is expected to be influenced by the degree of similarity to either of these prototypes.
...
\footnote{Such associations between constructions and words or lemmas are the main object of study in approaches such as Collexeme Analysis \citep{GriesStefanowitsch2004}.}
...
In (\ref{ex:determiners:nac}), the measure noun is modified by a cardinal \textit{drei} `three', and hence the \NACa\ (kind NP shows case agreement) is preferred.
In (\ref{ex:determiners:pgc}), the measure noun is modified by a non-cardinal determiner \textit{einige} `some', and the \PGCa\ (kind NP is in the genitive) is preferred.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:determiners:nac}\gll [[Drei Löffel]\Sub{Nom} [heißer Rum]\Sub{Nom}]\Sub{Nom} sind genug.\\
				     ...
				     \glt Three spoonful of hot rum are enough.}
    \ex{\label{ex:determiners:pgc}\gll [[Einige Löffel]\Sub{Nom} [heißen Rums]\Sub{Gen}]\Sub{Nom} sind genug\\
				     ...
				     \glt A few spoonful of hot rum are enough.}
  \end{xlist}
\end{exe}

The statistical models presented in Section~\ref{sec:corpusstudies} will be specified in a way that they could detect such a preference.

Finally, it has often been iterated that the \PGCa\ is more typical of higher registers or even exclusive to written language (see \citealp[320--323]{Hentschel1993}, where she also refers to descriptive and normative grammars).
This is not surprising in as much as the genitive -- an intrinsic part of the PGC -- is generally underrepresented in colloquial vernacular variants of German as a result of a diachronic process wherein many (but not all) uses of the genitive are replaced by other cases or periphrastic constructions \citep{FleischerSchallert2011}.
...
\begin{enumerate}[i.]
  ...
  \item Measure nouns quantified by cardinals favour the PGC variant.
...
However, if it occurs in a prototypical context and with prototypical lexical material, reading times drop ($\beta_{\text{ConstructionPGCa:Modelprediction}}=-0.132$).
