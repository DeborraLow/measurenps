...
\begin{document}
  ...
  \abstract{In this paper, a case alternation occurring in German measure noun phrases is examined from the perspective of cognitively oriented corpus linguistics.
    In pseudo-partitive constructions such as \textit{eine Tasse guter Kaffee} `a cup of good coffee', the embedded kind-denoting noun either agrees in its case with the measure head noun or it stands in the genitive as in \textit{eine Tasse guten Kaffees} with no difference in meaning.
    This alternation occurs only under specific syntactic conditions.
    The analysis is formulated in terms of Prototype Theory. 
    I assume that the alternants are modelled after non-alternating neighbouring constructions, which represent the prototypes more directly.
    ...
    Another major assumption is that the genitive construction is prototypical of more strongly grammaticalised measure nouns.
    I present a large-scale corpus study using the DECOW web corpus which supports the theory.
    In the statistical analysis of the corpus study, it is discussed whether Bayesian estimators lead to substantially different results than Likelihood Maximisation methods in this and similar studies.
    The model parameters are estimated with both methods for comparison.
    Finally, a forced choice experiment and a self-paced reading experiment are reported.
    The usage-based findings from the corpus study show a correlation with the behaviour of cognitive agents in both experiments.
