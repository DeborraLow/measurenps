...
\footnote{It would be more technically appropriate -- but also more cumbersome -- to speak of the \textit{kind noun phrase} because the embedded noun can be modified, cf.\ below.}
...
In a similar vein, \cite[284]{Brems2003} also considers nouns as measure nouns \textit{which, strictly speaking, do not designate a `measure', but display a more nebulous (sic!) potential for quantification}.

Descriptively, three different situations need to be distinguished \wrt case assignment in German measure noun phrases.
If the kind noun comes with a determiner, the construction resembles (and is usually called) a \textit{pseudo-partitive} (on partitives and pseudo-partitives, e.\,g., \citealp{Barker1998,Selkirk1977,Stickney2007,Vos1999}; for a recent application of the terminology to German, see \citealp{Gerstenberger2015}).%
\footnote{Wheras partitives are constructions denoting a proper part-of relation such as \textit{a sip of the wine}, pseudo partitives -- albeit syntactically similar and diachronically related to partitives in many languages -- merely denote quantities such as \textit{a sip of wine}.
...
While only the accusative is shown, case agreement of the kind noun is obligatory also with nominative and dative measure nouns.
...
\footnote{Some more normative grammars take stronger positions w.\,r.\,t. the grammaticality of these two options.
  See \cite{Hentschel1993,Zimmer2015} for analyses of the stances taken in more or less descriptive or normative grammars of German.
  As the usage and experimental data presented below should render it clear, there might be preferences under certain circumstances, but we cannot assume either construction to be \textit{ungrammatical} or -- for readers who reject the notion of grammaticality: \textit{(virtually) unanimously unacceptable}.}
...
I now turn to some subtler points related to the measure noun case alternation, namely:

\begin{enumerate}
  \item{\label{it:intro:datsg} alternative forms of dative singular neuter adjectives}
  \item{\label{it:intro:femsg} case syncretism in feminine NPs}
  \item{\label{it:intro:plurl} the parallel plural/collective construction(s)}
...
As for \ref{it:intro:datsg}, a complication with neuter kind nouns in the dative is mentioned by \cite[20--22]{Zimmer2015}.
He reports a high number of occurrences of adjectives being inflected according to the \textit{weak} inflectional pattern, which is normally only used if a strongly inflected determiner precedes the adjective: \textit{rot-en Wein} instead of \textit{rot-em Wein}.
In my corpus sample (to be made available after publication of this paper), this tendency was not nearly as clear, and there was a high number of very noisy sentences among those potentially showing this pattern.
...
I treat them like any other measure noun because they enter into both the NAC as in (\ref{ex:uninflectedmeasures:a}) and the PGC as in (\ref{ex:uninflectedmeasures:b}).
...
This section therefore reviews some existing analyses.
