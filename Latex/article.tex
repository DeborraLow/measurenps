...
In this section, I briefly motivate the choice of statistical models which I use later in Section~\ref{sec:corpusstudies}, namely Generalised Linear Mixed Models (GLMMs) using Maximum Likelihood Estimation.%
\footnote{It would be technically more precise to speak of Hierarchical Logistic Regression rather than a GLMM in the case of this study.}
Many readers might think that the use of this class of statistical methods in the modeling of grammatical alternations does not require much motivation.
After all, GLMMs have been established as the major tool in the analysis of (\citealp{BresnanEa2007,BresnanHay2010,BresnanFord2010,DivjakArppe2013,Gries2015,NessetJanda2010}, to name only a few publications).
However, over the past years alternative methods have been proposed.
Within the Generalised Linear Modeling framework, multimodel averaging and multimodel inference (\citealp{BurnhamAnderson2002}, linguistic applications include \citealp{BarthKapatsinski2014,KupermanBresnan2012}) have been suggested to deal with model uncertainty and the sometimes problematic necessity to settle on one specific model structure.
Also, alternative estimators have been introduced into the linguistics literature, most notably Bayesian estimation (a comprehensive introduction in \citealp{GelmanEa2014}, see \citealp{Levshina2016,Divjak2016} for praises by linguists).
On the other hand, completely different modeling approaches have been proposed, for example Trees \& Forests \citep{StroblEa2009} and Naive Discriminative Learning \citep{Baayen2011,MilinEa2016}.
The last two methods are summarised and compared to regression methods in \cite{BaayenEa2013}, and \cite{TheijssenEa2013} provides a similar comparison with even more methods.

Often, linguists provide convincing arguments why adopting a statistical tool might beneficial.
For example, \citep{Baayen2011,MilinEa2016} argue that Naive Discriminative Learning (NDL) is more adequate for modeling in strands of linguistics where cognitive reality is desired.
For example, the fact that regression models cannot properly deal with redundancy in the form of multicollinearity makes them implausible given some cognitive research, while NDL can deal with redundant information \citep[508]{MilinEa2016}.
Furthermore, the fact that likelihood maximisation is a form of optimsation is criticised \citep[508]{MilinEa2016}, although in NDL and Trees \& Forests are praised for \textit{arriv}[\textit{ing}] \textit{at their optimal solutions on their own} \cite[256]{BaayenEa2013}.
More fundamentally, regression modeling has been criticised because it usually relies on high-level linguistic abstractions, and NDL (or other methods) can achieve at least the same adequacy by considering low-level units such as words or even character ngrams <++>
...
Finally, it has often been iterated that the \PGCa\ is more typical of higher registers or even exclusive to written language (see \citealp[320--323]{Hentschel1993}).
