...
For the masculine\slash neuter model (henceforth \textit{MN model}), the final sample size was $n_{\text{MN}}=3,949$ with 672 cases of \PGCa\ and 3,277 cases of \NACa.
For the feminine model (henceforth \textit{F model}), it was $n_{\text{F}}=1,114$ with 462 cases of \PGCa\ and 652 cases of $\NACa$.
...
For the response variable, \PGCa\ was coded as $1$, such that coefficients leaning to the positive side can be interpreted as favouring the \NACa.
...
Standard diagnostics show that the model quality is quite high.
For the MN model, Nakagawa \& Schielzeth's pseudo-coefficient of determination is $R_m^2=0.495$ and $R^2_c=0.544$ (see \citealp{Gries2015} for a linguist-friendly introduction to these $R^2$ measures, or else \citealp{NakagawaSchielzeth2013}).
The rate of correct predictions is $0.872$, which means a proportional reduction of error of $\lambda=0.247$.
For the F model, $R_m^2=0.468$ and $R^2_c=0.574$ are achieved, as well as a correct prediction rate of $0.815$, which means a proportional reduction of error of $\lambda=0.554$.
The measure and kind lemma intercepts are distributed with the following standard deviations:
MN model $\sigma_{\text{Measurelemma}}<0.001$ (131 lemmas), $\sigma_{\text{Kindlemma}}=0.591$ (68 lemmas);
F model $\sigma_{\text{Measurelemma}}=0.905$ (73 lemmas), $\sigma_{\text{Kindlemma}}=0.000$ (19 lemmas).
In the MN model, variance between individual measure lemmas do not play much of role, mostly because the second-level regressors successfully model the between-lemma variance.
In the F model, kind lemmas (and -- as will be shown -- the second level model for kind lemmas) could not be estimated with great success, most likely because of the low number of levels.%
...
In Table~\ref{tab:bigtable}, there are groups of columns for the results obtained from Maximum Likelihood estimation and Markov-Chain Monte Carlo estimation, both for the MN model and F models and including confidence intervals.
...
The resulting coefficients and intervals as well as an * for \textit{interval does not contain $0$} (column \textit{Not 0}) are also given in Table~\ref{tab:bigtable} in the groups of columns labelled \textit{MCMC}.
...
Before I turn to some details, it should be noticed that the results from standard MLE (with bootstrap) and MCMC (with default priors) obviously differ in numerical details -- but not enough to result in different interpretations.
...
In only one single case (MN model, \textit{Measureattraction}) the MLE bootstrap confidence interval does not include $0$ while the MCMC interval does (barely, with $-0.01$).
While I only interpret the MLE results, virtually the same interpretation could be made for the MCMC results.
Furthermore, the MN model and the F model agree in many relevant aspects.
...
\begin{figure}[h!]
  ...
  \caption{Effect plot for the regressor \textit{Kindattraction} for the MN model and the F model (y axes have different scales)}
...
\begin{figure}[h!]
  ...
  \caption{Effect plot for the regressor \textit{Measureattraction} for the MN model and the F model (y axes have different scales)}
  \label{fig:eff:measureattraction}
\end{figure}

First, the prototypicality effect related to the non-alternating \PGCd\ and \NACb\ can be shown, see the effect plots in Figure~\ref{fig:eff:kindattraction} and Figure~\ref{fig:eff:measureattraction}.%
...
Only the F model \textit{Kindattraction} regressor is clearly useless (`non-significant') in the PBmodcomp test, and the effect plot shows a corresponding picture.
This is related to the fact that in the F model the kind lemma effect and the corresponding second-level model could not be estimated successfully (remember that $\sigma_{\text{Kindlemma}}=0.000$).
The effect plots show a picture in line with a \textit{divide by 4} interpretation of the coefficients as shown in Table~\ref{tab:bigtable}.%
\footnote{Coefficients are on the logit scale, which does not linearly correspond to the probability scale.
If coefficients are reasonably small, however, dividing the coefficient by $4$ gives a good approximation of the change on the probability scale for each increment of the response variable \citep[82]{GelmanHill2006}.}
Given that all regressors have been z-transformed, we can make statements such as the following:
If a masculine kind noun is one standard deviation more frequently encountered in the \PGCd\ (compared to its frequency in the \NACb) than the kind nouns on average, the probability that the \PGCa\ is chosen instead of the \NACa\ with this kind noun in it rises by $\beta^{MN}_{\text{Kindattraction}}/4=0.49/4=0.1225$, etc.
For measure nouns in MNPs with masculine kind nouns, it is $\beta^{MN}_{\text{Measureattraction}}/4=0.21/4=0.0525$, and for measure nouns in MNPs with feminine kind nouns, it is $\beta^{F}_{\text{Measureattraction}}/4=0.43/4=0,1075$.
As readers can observe, these numbers correspond approximately to the slopes of the effect plots.
...
\begin{figure}[h!]
  ...
  \caption{Effect plot for the regressor \textit{Measureclass} for the MN model}
  \label{fig:eff:measureattraction}
\end{figure}

In Section~\ref{sec:analyses}, it was also hypothesised that there may be effects specific to classes of lemmas and classes of lemmas, and that they might be related to degrees of grammaticalisation of the measure noun and\slash or semantic prototypicality effects.
The picture is clear for the MN model.
Lemma-specific effects were not detected ($\sigma_{\text{Measurelemma}}<0.001$), but instead the lemma class second-level predictor was successfully estimated ($p=0.004$ in the PB test).
The large confidence intervals around the estimates (between $15.6$ and $28.37$) indicate that maybe the sample size needs to be higher, or a slightly modified classification might be even more adequate.
Looking at the effect plot in Figure~\ref{fig:eff:measureattraction}, however, it is evident that the more abstract physical measure nouns with a high degree of grammaticalisation favour the \NACa\ (\textit{Square}, \textit{Length}, \textit{Currency}, \textit{Capacity}, \textit{Weight}) whereas the more concrete and less grammaticalised classes of measure nouns (which usually have an alternative use outside of MNPs) favour the \PGCa\ (\textit{Layer}, \textit{Amount}, \textit{Vessel}, \textit{Portion}).
Ideally, there should be more differentiation in the second group, but given the large confidence intervals, significantly larger samples would be needed for more precise estimation.
As mentioned before, the second level model for measure nouns in the F model could not estimated with meaningful results.

On the other hand, the heuristic semantic classification for kind nouns in \textit{Kindconsistency} is much less useful.
The MN model did not even converge with this regressor, and for the F model an insufficient $p=0.07$ was achieved in the PB test.
Also, \textit{Kindint} does not contribute anything useful in either model (MN model $p=0.432$; F model $p=0.326$).
The only regressor that has a considerable influence is \textit{Kindedible}, \ie, whether the substance denoted by the kind noun is edible.
Since these regressors were added heuristically, there is no interpretation in terms of substantive theory, however, and I am hesitant to count it as very meaningful.
In sum, no consistent picture emerges for kind noun-specific semantic prototypicality effects, but for measure nouns there is a clear indication that -- as predicted -- classes of more strongly grammaticalised nouns occur proportionally more frequently in the \PGCa.
...
\begin{figure}[h!]
  ...
  \caption{Effect plot for the regressor \textit{Minus1pos} for the MN model and the F model (y axes have different scales)}
  \label{fig:eff:measureattraction}
\end{figure}

I now turn to the predicted effect of plural measure nouns and cardinals as quantifiers.
Plurality has a clear effect in the expected direction (plural favours the \PGCa) in the MN model ($p=0.002$ for \textit{Measurenumber}).
In the F model, the effect has the same direction, but the estimate is quite imprecise ($p=0.160$), again likely due to the low sample size.
Figure~\ref{fig:eff:measureattraction} shows, however, that the left context of the measure noun influences the choice of the variant (in both models, the regressor has $p=0.002$).
...
Increased \textit{Badness} favours the \NACa\ (only in the MN model $\beta=-0.2$, $p=0.002$), and so does a lesser density of genitives (MN model $\beta=-0.66$, $p=0.004$; F model $\beta=-0.52$, $p=0.002$).
A higher length of the NP (\textit{Matchlength}) favours the \PGCa\ (MN model $\beta=0.54$, $p=0.002$; F model $\beta=0.63$, $p=0.002$) as predicted, but abbreviated measure nouns (\textit{Measureabbreviated}) do not seem to have much of an effect (MN model $p=0.730$; F model $p=0.267$).
...
Finally, the nuisance variables \textit{Measurefreq} and \textit{Kindfreq} have no clear effect, except for maybe a $p=0.041$ for \textit{Measurefreq} in the MN model.
The coefficient is $-0.16$ meaning that with each increase of the lemma log-frequency (per one million tokens) of the measure noun of one standard deviation, the probability that the \PGCa\ is chosen is lowered by approximately $-0.16/4=-0.04$.
Since token frequency is no direct proxy for grammaticalisation, I hesitate to interpret this effect.
The effect for \textit{Measurecase} is as predicted by previous studies (see Section~\ref{sec:analyses}).
The dative favours the \PGCa\ in the MN model with $p=0.002$ and a coefficient for the dative (vs.\ the nominative, which is on the intercept) of $0.63$.
In the F model, \textit{Dative} was not a level of \textit{Measurecase} due to the syncretisms mentioned in Section~\ref{sec:descriptive}.
