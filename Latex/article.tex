...
Readers might think that the method used here for modeling grammatical alternations -- namely Hierarchical Logistic Regression\slash Generalised Linear Mixed Models (GLMMs) -- does not require much motivation.
...
What makes frequentism attractive is that it lends itself to a theory of testing whether a theoretical prediction (a fixed model) was successful or not.
Such tests, if passed, contribute to the overall well-testedness of the theory.
Bayesian approaches, on the other hand, condition on the data and quantify inductively (in the standard interpretation) the probability of models given specific observations.
The models are thus not fixed, and the probability is usually equated with researchers' posterior beliefs about models or model parameters.
There is actually a debate among Bayesians whether and how a notion of hypothesis testing is compatible (or even already contained) in the Bayesian approach.
In \citet[10]{GelmanShalizi2013}, the authors -- prominent Bayesians -- acknowledge that theory testing is a desideratum, and they state about the standard inductive interpretation of Bayesianism that \textit{most of this received view of Bayesian inference is wrong}, and they develop a Bayesian notion of p values (see also \citealp{Mayo2013}, for a frequentist reply; also \citealp{Senn2011} on different strands of Bayesianism and their stance on inductive vs.\ deductive reasoning).
...
Frequentist methods were, however, never designed as cognitive models, but as systems of inference for researchers (see above).
The actual question that emerges here is how we interpret our statistical models (estimated on corpus data).
...
In the former case, we should definitely resort to methods like NDL.
...
I view my own work (see Sections~\ref{sec:germanmeasurenps}--\ref{sec:externalvalidation}) in the tradition of testing theories that embrace high-level generalisations in a probabilistic framework.
...
\footnote{In fact, she admits that changing the priors did not lead to substantially different results in her footnote 8.
  This is a clear sign that the prior is \textit{swamped by the data} (QUOTE??).
  Relating back to cognitive plausibility, this means that in the case at hand Bayesian methods cannot be more cognitively plausible than Maximum Likelihood Estimation.}
...
\footnote{
  It could be argued that some more advanced measure of attraction strength should be used, as is done in Collostructional (or Collexeme) Analysis \citep{GriesStefanowitsch2004}.
