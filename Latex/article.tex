...
A random effect for the sentence was added to catch idiosyncrasies of single sentences (or badly selected sentences).
Also, a random effect for the participant was added.
...
A certain amount of the variance can be accounted for by idiosyncrasies of single sentences ($\sigma_{\text{Sentence}}=0.599$, $16$ levels).
Also, among participants, there are clearly different preferences ($\sigma_{\text{Participant}}=0.407$, $24$ levels).
...
However, the regressor \textit{Modelprediction} still achieves $p_{\text{PB}}=0.001$ ($1,000$ replications) and is estimated at $4.555$ relative to an intercept of $-1.511$.
The confidence interval from a parametric bootstrap ($1,000$ replications) for the regressor is acceptable with a lower bound of $2.746$ and an upper bound of $6.330$.
Nakagawa \& Schielzeth's pseudo-coefficients of determination are $R^2_{m}=0.271$ and $R^2_{c}=0.442$, which means that roughly 27\% of the variance in the data can be explained by considering the predictions from the corpus-based GLMM.
...
Finally, I report the results of a self-paced reading experiment also designed to test whether the statistical model derived from usage data (see Section~\ref{sec:corpusnonhierarchicalmodel}) can be used to predict speakers' behaviour.
As opposed to the forced choice experiment reported in the last section, self-paced reading is completely implicit (see Section~\ref{sec:cogocl} for more on the motivation).

I used exactly the same stimuli as in the forced choice experiments.
Each participant read the sentence with the variant preferred by the corpus model and the modified sentence with the dispreferred variant.
To avoid and\slash or randomise repetition effects, the stimuli for each participant were separated into two blocks of $16$ targets and $33$ fillers.
It was made sure that from each sentence pair, one sentence was assigned to the first block, and the other sentence to the other block.
This assignment was randomised for each participant, and the order within the block was also randomised while keeping the sentences from each pair as far apart as possible.
