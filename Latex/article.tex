...
\footnote{Whereas partitives are constructions denoting a proper part-of relation strictly requiring a definite kind noun as in \textit{a sip of the wine}, pseudo partitives -- albeit syntactically similar and diachronically related to partitives in many languages -- merely denote quantities and contain indefinite kind nouns as in \textit{a sip of wine}.
In the literature on German, some authors incorrectly call the pseudo-partitive a \textit{partitive} \cite{Hentschel1993} while some realise the difference and at least mention it \cite{Eschenbach1994,GallmannLindauer1994,Loebel1989,Zimmer2015}.}
I use \textdagger\ to denote general unacceptability, and the acceptability judgements given in (\ref{ex:intro:pseudopartitive}) are completely undisputed.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[\textdagger]{\label{ex:intro:pseudopartitive2} Wir trinken [[eine Tasse]\Sub{Acc} [einen leckeren Kaffee]\Sub{Acc}]\Sub{Acc}.}
  \end{xlist}
\end{exe}

When the kind noun is bare -- \ie\ when it comes without a determiner and without modifying adjectives -- the kind noun has to agree in case with the measure noun, and the genitive seen in the PGC is not acceptable, see (\ref{ex:intro:narrowapposition}).
This construction is usually classified as a \textit{Narrow Apposition Construction} \citep{Loebel1986}, henceforth NAC.%
\footnote{Notice that the unavailability of the genitive on the kind noun can be seen as following from a quirky constraint that genitive NPs in German require the presence of some strongly case-marked element (determiner or adjective) in addition to the head noun in order to be acceptable \citep{GallmannLindauer1994,Schachtl1989}.}

\begin{exe}
  \ex\label{ex:intro:narrowapposition}
  \begin{xlist}
    \ex[\textdagger]{\label{ex:intro:narrowapposition1} Wir trinken [[eine Tasse]\Sub{Acc} [Kaffees]\Sub{Gen}]\Sub{Acc}.}
...
While only the accusative is shown in (\ref{ex:intro:narrowapposition}), the kind noun obligatory agrees in case also with nominative and dative measure nouns.
...
\footnote{Some descriptive and normative grammars take stronger positions w.\,r.\,t.\ the acceptability of these two options.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[\textdagger]{\label{ex:analyticalpartitive:b}\gll ein Glas von rotem Wein\\
                                                a glass of red wine\\
...
\begin{exe}
  ...
  \ex{\label{ex:mit}\gll    ein Glas mit rotem Wein\\
			    ...
			    \glt a glass with red wine in it}
  \end{xlist}
\end{exe}
...
While the constructions and the alternation are often covered in descriptive grammars such as \citet[XYZ]{Eisenberg2013a}, the available linguistic literature contains little specific discussion of the situation in German.
...
The alternating constructions in German are both clearly pseudo-partitives, but the grammaticalisation paths assumed by \cite[esp.\ 526--530]{Koptjevskaja2001}, which I interpret freely here, are clearly pertinent.
The development usually begins with constructions involving two referential nouns (not necessarily forming a single NP) and a \textit{separative} meaning as in \textit{(cut) two slices from the cake} \citep[535]{Koptjevskaja2001}.
...
The pseudo-partitive stage finally instantiates a \textit{quantity-of} relation, potentially even leading to fully grammaticalised quantifiers as in English \textit{a lot}.
Since in German, the PGC is clearly the older variant \citep{Zimmer2015} with the overt \textit{quantity-of} marker in the form of the genitive suffix and at least the potential to form a true partitive (if the kind noun is definite), which the NAC lacks, we can expect more strongly grammaticalised measure nouns to prefer the NAC.
Since clearly non-referential nouns like \textit{Gramm} `gram' and \textit{Meter} `metre' should favor the NAC according to this hypothesis, the effect can be detected by including information about measure noun classes in the corpus-based models in Section~\ref{sec:corpusstudies}.
...
The following analysis of the morpho-syntax of German MNPs will however, provide hypothetical cues along similar lines.
...
It cannot be understood properly without taking into account the strong\slash weak inflection patterns of adjectives described in Section~\ref{sec:descriptive} (especially Footnote~\ref{fn:strongweak}).
...
\footnote{The structural cases (nominative and accusative) in the singular are marked with \textit{-e}, the oblique cases (dative and genitive) as well as the entire plural are marked with \textit{-en}.
...
Thus, the adjectives in the target constructions have properties of adjectives as well as determiners in a sense:
they are lexical adjectives and function as attributive modifiers, but they are inflected as determiners and are the leftmost element in the NP.
This unusual double nature of adjectives in NPs without a determiner leads to a plausible interpretation of the pattern shown in Table~\ref{tab:constructions}.
Whenever speakers classify the adjective in the kind noun phrase as a determiner, they have to use the PGC because, if there is a determiner, the PGC is the only option.
When they classify the adjective as an adjective, the kind NP has no determiner, and the NAC is the only option.%
...
In a framework that assumes similarity-based probabilistic classification, such as Prototype Theory, we can now regard the PGC with a determiner in the kind noun phrase as instantiating the PGC prototype and the NAC with just a bare kind noun as the NAC prototype.
...
The situation is made difficult by the fact that the semantic differences between the variants are at best subtle, which means the prototypes are not associated with nice and clearly detectable differences in meaning.
However, both the kind noun lemmas and the measure noun lemmas probably appear with different relative frequencies in the NAC with a bare kind noun and the PGC with a determiner inside the kind noun phrase (in part possibly due to their meaning and usage).
...
In other words, lemmas themselves might have preferences for one prototype or the other.
...
The best evidence for the relevance of this factor in an ongoing grammaticalisation process is that plural kind nouns almost always trigger PGC and virtually never NAC (Section~\ref{sec:descriptive}).
More indirectly, plurality of the measure noun could favor the PGC over the NAC, and exact quantification in the form of cardinals even more so.%
\footnote{The effect of the plural was empirically shown but not explained in \cite{Hentschel1993}.}
...
The former is highly restricted for \textit{zwei Haufen französischer Watte} `two heaps/\textdagger lots of French cotton' (GPC).
...
In other words, a precisely quantified measure noun leads to a higher individuation (the creation of portions) of the substance denoted by the kind noun, which is more appropriate for the PGC prototype.%
\footnote{In the most extreme, some measure nouns cannot be pluralised, such as \textit{eine Menge} `a lot of'.
...
It was made clear from the beginning and throughout that the preferences are indeed subtle.}

Finally, it is claimed that the PGC is more typical of higher registers or even exclusive to written language (see \citealp[320--323]{Hentschel1993}, where she also refers to descriptive grammars).
...
There is no reason to assume that such preferences for cannot be part of the construction prototypes.
...
To summarise the discussion and core hypotheses, it is expected that:
...
\begin{enumerate}[i.]
  ...
  \item MNPs with masculine dative kind nouns disfavour the GPC comparatively less
  \item pluralic measure nouns and measure nouns quantified by cardinals favor the PGC variant
  \item .
\end{enumerate}
...
\subsection{A prelude to the statistical analysis: There are no right statistics!}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/corpus/04_glmm_fixef_firstlevel}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/corpus/04_glmm_fixef_secondlevel_kind}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/corpus/04_glmm_fixef_secondlevel_measure}
