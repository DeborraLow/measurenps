...
\begin{figure}[hb!]
  ...
  \caption{Coefficients (MLE and MCMC) with 95\% confidence intervals (for details see text); intercept (\textit{Leftcontext=C}, \textit{Measurecase=Nom}, \textit{Measurenumber=Sg}, \textit{Kindgender=Masc}, \textit{Measureclass=Physical}; 0 for all numeric z-transformed regressors) is -4.328 (MLE) and -4.441 (MCMC)}
  \label{fig:fixeffs}
\end{figure}
...
A higher than average lemma frequency of measure nouns favours the \NACa\ as expected ($\beta_{\text{Measurefreq}}=-0.257$, $p_{\text{PB}}=0.003$).
With kind nouns, higher frequency seems to favour the \PGCa ($\beta_{\text{Kindfreq}}=0.161$, $p_{\text{PB}}=0.066$).
...
Both experiments use sentences containing attested MNPs from the corpus sample embedded into radiacally simplified sentences, and the probabilities that the corpus-based statistical model (see Section~\ref{sec:corpushierarchicalmodel}) assigns to the competing constructions in these sentences.
...
As stimuli, attested MNPs from the corpus study were used, but the sentences were radically simplified to avoid .
...
For example, the case, number, etc. of the MNP remained the same, as well as the choice of lexical material within the MNP.
...
Then, a multilevel logistic regression was specified with the probability of the \PGCa\ predicted for each sentence by the corpus-based GLMM (\textit{Modelprediction}) as the only fixed effect.%
...
\footnote{The random slopes were added to comply with \citet[257]{BarrEa2013} who predict \textit{catastrophically high Type I error rates} for designs with within-subject manipulations if random effects structures are not kept maximal.
...
The regressor \textit{Modelprediction} achieves $p_{\text{PB}}=0.002$ ($1,000$ replications) and is estimated at $4.500$ relative to an intercept of $-1.520$.
The confidence interval from a parametric bootstrap ($1,000$ replications) for the regressor is acceptable with a lower bound of $2.738$ and an upper bound of $6.390$.
Nakagawa \& Schielzeth's pseudo-coefficients of determination are $R^2_{m}=0.267$ and $R^2_{c}=0.435$, which means that roughly 27\% of the variance in the data can be explained by considering the predictions from the corpus-based GLMM.
...
It is expected that reading the \textit{un}expected or less prototypical variant (the one assigned a low probability by the corpus-derived model) in a given context and with given lexical material might incur a processing overhead \citep{Kaiser2013}.
...
Each participant read both the $16$ sentences with the variant predicted by the corpus model and the $16$ modified sentence with the variant that the corpus model would not predict.%
...
The fillers also came in pairs such that the second block contained only sentences to which participants had been exposed in the first block in slightly modified form.
...
Since the corpus GLMM predicts the probability of the \PGCa, it is expected that reading times are positively correlated (longer reading times) with the probability if the stimulus actually contains the \NACa, and negatively correlated (shorter reading times) if the stimulus actually contains the \PGCa.
...
Random intercepts were specified for \textit{Participant} and \textit{Item} (the $16$ sentence pairs are one \textit{Item}).%
\footnote{I tried random slopes, but it was impossible to get the algorithm to converge.}
...
\begin{table}
  ...
  \end{tabular}
  \caption{Fixed effect coefficient table for the LMM used to analyse the self-paced reading experiment}
  \label{tab:exp:spr}
\end{table}

Table~\ref{tab:exp:spr} shows the coefficient estimates with a 95\% parametric bootstrap confidence interval ($1,000$ replications).
...
Comparing the full model to a model without the main regressor \textit{Modelprediction} (and consequently also the interaction with \textit{Construction}) in a PB test gives $p_{\text{PB}}=0.018$.
...
While the estimate for the sentences with \NACa\ is clearly too imprecise, there is a clear tendency in the sentences with \PGCa, which is also confirmed by the `significant' results from the bootstrapped confidence intervals (see Table~\ref{tab:exp:spr}) and from the PB test reported above.
...
However, if it occurs in a prototypical situation, reading times drop ($\beta_{\text{ConstructionPGCa:Modelprediction}}=-0.132$), which is also reflected in the slope of curve in the right panel of Figure~\ref{fig:spr:effects}.
...
In this context, it should be remembered from Section~\ref{sec:gettingdata} that even the \PGCd\ is much rarer that the \NACb\ ($17,252$ vs.\ $315,635$ occurrences in the auxiliary corpus samples).
...
This possibility will be considered, among other things, in the next section.
