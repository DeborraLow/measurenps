...
In the case at hand, three different syntactic configurations need to be distinguished with regard to case assignment inside German MNPs.
...
If the kind noun is bare -- \ie\ if it comes neither with a determiner nor a modifying adjective -- it has to agree in case with the measure noun as in (\ref{ex:intro:narrowapposition1}), and the genitive as seen in the PGC is not acceptable, see (\ref{ex:intro:narrowapposition2}).

\begin{exe}
  \ex\label{ex:intro:narrowapposition}
  \begin{xlist}
    \ex[ ]{\label{ex:intro:narrowapposition1}\gll Wir trinken [[ein Glas]\Sub{Acc} [Wein]\Sub{Acc}]\Sub{Acc}.\\
...
%While only an accusative is shown in (\ref{ex:intro:narrowapposition1}), the kind noun obligatory agrees in case also with nominative and dative measure nouns.

The alternation can be observed \textit{only when the kind noun occurs with an attributive adjective but without a determiner}, as in (\ref{ex:intro:alternation}), where both the PGC in (\ref{ex:intro:alternation1}) and the NAC in (\ref{ex:intro:alternation2}) are equally acceptable.
...
As the usage and experimental data presented below (especially Section~\ref{sec:exp:fc}) should render it clear, there might be preferences, but we cannot assume either construction to be unacceptable.
}
...
\footnote{Plurals like \textit{Pfunde} and \textit{Prozente} have special meanings and have very restricted uses, mostly in idiomatic expressions and prefabs such as \textit{Pfunde verlieren} `lose pounds' and \textit{Prozente machen} `make a profit'.
...
fourth, preferences in different styles.
...
%Furthermore, the internal structure of MNPs can only be understood if the strong\slash weak inflection patterns of adjectives are taken into account.
...
In a probabilistic framework like Prototype Theory, the crucial question (beyond simple lemma preference effects) is, however, which factors control speakers' decisions to use either alternant. 
In the remainder of this section, I argue that the NAC and the PGC are prototypes associated with different degrees of the grammaticalisation of the measure noun and related morpho-syntactic properties, as well as stylistic effects.
...
Conversely, the NAC lacks this ability to form true partitives, and it thus forms the more typical environment for expressing measurement (as opposed to partitivity).
Hence, we can expect the \NACa\ to be a typical hosting construction for more strongly grammaticalised measure nouns.
...
This is most obviously so with the highly grammaticalised physical measure nouns like \textit{centilitre} which are very typically used with exact numeral instead of unspecific quantifiers, although both options are available (\textit{three centilitres} vs.\ \textit{several centilitres}).
...
This hypothesis will be reflected in the specification of the statistical model presented in Section~\ref{sec:corpusstudies}.

Fourth and finally, it was found that the \PGCa\ is more typical of higher stylistic levels (distinctly edited, closer to the non-regional standard, more formal) and\slash or even exclusive to written language (see \citealp[320--323]{Hentschel1993}).
...
Under an integral view of prototypes, which incorporate effects related to larger contexts, registers, and styles, such preferences can be part of what defines the construction prototypes.
In the corpus study in Section~\ref{sec:corpusstudies}, stylistic effects will therefore be modelled -- even if only using two very simple proxy variables.

To summarise, I assume that the alternation is controlled by the following factors:
...
\begin{enumerate}[i.]
  ...
  \item The \PGCa\ is associated with higher stylistic levels.
\end{enumerate}
