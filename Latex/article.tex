...
If the kind noun is bare -- \ie\ if it does neither come with a determiner nor a modifying adjective -- it has to agree in case with the measure noun, and the genitive seen in the PGC is not acceptable, see (\ref{ex:intro:narrowapposition}).
...
In the case at hand, the variable might be something like \textit{measuring substances and collections}.
I argue that it is fully justified to focus narrowly on \NACa\ and \PGCa\ with their well-defined morph-syntactic properties, mostly because the alternative constructions are not used in the same range of contexts.
...
I will argue below that the prototypes differ mainly in that they attract different (classes of) measure nouns more or less strongly, mostly because of degrees of grammaticalisation.
However, if lemmas are attracted with different strengths by the two prototypes, then we can expect a baseline attraction effect:
nouns which occur more often in the \PGCd\ should favour the \PGCa, and nouns which occur more often in the \NACb\ should favour the \NACa.
Put differently, a noun's relative frequencies in the prototypical non-alternating constructions should be indicative of same noun's preference to appear in the two alternating variants.
This baseline attraction effect will be quantified in the corpus study Section~\ref{sec:corpusstudies}.

Turning to grammaticalisation, it is often assumed that pseudo-partitives arise as a form of grammaticalised partitives (\eg\ \citealp[536--539]{Koptjevskaja2001} for Finnish and Estonian, \citealp[559]{Koptjevskaja2001} for European languages per se).
%
...
%}
%
The alternating constructions in German are both clearly pseudo-partitives, but the grammaticalisation paths uncovered by \cite[esp.\ 526--530]{Koptjevskaja2001} are relevant for an analysis of the alternation.
...
In German, the PGC is clearly the older variant \citep{Zimmer2015}, still having an overt \textit{quantity-of} marker in the form of the genitive suffix.
...
The statistical models in Section~\ref{sec:corpusstudies} will be designed to detect such an effect.
...
The level names stand for (again with their frequency in the final sample) \textit{cardinal} ($f=1,939$), \textit{determiner} ($f=1,971$), \textit{other} ($f=221$), \textit{adjective} ($f=845$), and \textit{preposition} ($f=87$).
...
\begin{figure}[hb!]
  ...
  \caption{Coefficients (MLE and MCMC) with 95\% confidence intervals (for details see text); the intercept (\textit{Leftcontext=C}, \textit{Measurecase=Nom}, \textit{Measurenumber=Sg}, \textit{Kindgender=Masc}, \textit{Measureclass=Physical}; 0 for all numeric z-transformed regressors) is -4.328 (MLE) and -4.441 (MCMC)}
  \label{fig:fixeffs}
\end{figure}
...
Generalised variance inflation factors for the regressors were calculated to check for multicollinearity \citep{FoxMonette1992,ZuurEa2010}, and none of the corrected $\text{GVIF}^{1/2\text{df}}$ was higher than $2$.
...
Only \textit{Kindfreq} ($\mpPB=0.066$) can be seen as slightly too high to be convincing (`non-significant').
...
If we were interested in significance at $\alpha=0.05$, then the decision based on the confidence intervals differs for \textit{Kindfreq} ($\mpPB=0.066$, MLE lower bound is $-0.008$ and MCMC lower bound is $0.009$).
...
Plurality has a clear effect in the expected direction (plural favours the \PGCa) ($\beta_{\text{MeasurenumberPl}}=0.814$, $\mpPB=0.001$ for \textit{Measurenumber}).
Figure~\ref{fig:eff:leftcontext} shows that the left context of the measure noun influences the choice of the variant ($\mpPB=0.001$).
Cardinals have a strong tendency to co-occur with the \NACa.
Both effects were predicted in Section~\ref{sec:analyses}.
...
Figure \ref{fig:afc:proportions} shows the relationship between the probabilities calculated from the corpus-based model and the choices made by participants.
The close correlation between the probabilities derived from usage data and the choices in the experiment are evident.
...
A certain amount of the variance can be accounted for by idiosyncrasies of single sentences ($\sigma_{\text{Sentence}}=0.861$, $\sigma^{\text{Modelprediction}}_{\text{Sentence}}=0.250$, $16$ levels).%
...
Also, among participants, there are clearly different preferences ($\sigma_{\text{Participant}}=0.530$, $\sigma^{\text{Modelprediction}}_{\text{Participant}}=0.076$, $24$ levels).
...
The regressor \textit{Modelprediction} achieves $\mpPB=0.002$ ($1,000$ replications) and is estimated at $4.500$ relative to an intercept of $-1.520$.
The confidence interval from a parametric bootstrap ($1,000$ replications, percentile method) for the regressor is acceptable with a lower bound of $2.738$ and an upper bound of $6.390$.
Nakagawa \& Schielzeth's pseudo-coefficients of determination are $R^2_{m}=0.267$ and $R^2_{c}=0.435$, which means that roughly 27\% of the variance in the data can be explained by considering only the predictions from the corpus-based GLMM.
The effect display for the single fixed regressor \textit{Modelprediction} is given in Figure \ref{fig:afc:effects}.
It confirms the effect descriptively shown in Figure~\ref{fig:afc:proportions}:
...
\footnote{Notice that lemmas and their frequencies as well as lemma classes are part of the corpus-based GLMM, and there was no additional controlling of lemma frequencies, etc.}
To avoid repetition effects, the stimuli for each participant were separated into two blocks of $16$ targets and $33$ fillers per block.
...
The assignment of members of the individual sentence pairs to one of the blocks was randomised for each participant individually, and so was the order within the block.
...
The adjective and kind noun (\ie\ the constituents bearing the critical case markers) were used as the target region, such as the bracketed words in the example \textit{zwei Gläser} [\textit{sprudelndes Wasser}] `two glasses of sparkling water'.
Outliers farther than 2 inter-quartile ranges from the mean logarithmised residual time were reamoved ($64$ data points), resulting in a total number of $n=1,152$ observations.
...
Furthermore, the position of the sentence in the individual experiment run was included as a fixed effect to control for increasing reading speed during experiment runs.
Random intercepts were specified for \textit{Participant} and \textit{Item} (the $16$ sentence pairs are one \textit{Item} each).%
\footnote{I tried random slopes in order to keep the random effect structure maximal \citep{BarrEa2013}, but it was impossible to get the algorithm to converge.}

\begin{table}
  \centering
  \begin{tabular}{lrrrc}
    Regressor & \multicolumn{1}{r}{Coefficient} & \multicolumn{1}{r}{CI low} & \multicolumn{1}{r}{CI high} & \multicolumn{1}{r}{0 in CI} \\ \midrule
    ConstructionPGCa                 &  0.062 &  0.023 &  0.103 &  *  \\ 
    Modelprediction                  &  0.025 & -0.065 &  0.122 &     \\ 
    Position                         & -0.005 & -0.005 &  0.004 &     \\ 
    ConstructionPGCa:Modelprediction & -0.132 & -0.229 & -0.041 &  *  \\ 
  \end{tabular}
  \caption{Fixed effect coefficient table for the LMM used to analyse the self-paced reading experiment; the intercept is 0.819}
  \label{tab:exp:spr}
\end{table}
...
The standard deviation of the participant intercepts is $\sigma_{\text{Participant}}=0.079$ and of the item intercepts $\sigma_{\text{Item}}=0.039$.
Comparing the full model to a model without the main regressor \textit{Modelprediction} (and consequently also without the interaction with \textit{Construction}) in a PB test gives $\mpPB=0.018$.
Nakagawa and Schielzeth's pseudo-determination coefficients are $R^2_m=0.238$ and $R^2_c=0.347$.
...
While the overall model quality is acceptable, the relevant effect is estimated with some uncertainty, which is also visible in the effect plot in Figure~\ref{fig:spr:effects}.
While the estimate for the sentences with \NACa\ is obviously very imprecise, there is a clearer tendency in the sentences with \PGCa, which is also confirmed by the `significant' results from the bootstrapped confidence intervals (see Table~\ref{tab:exp:spr}) and from the PB test reported above.
The \PGCa\ brings about an increased reading time ($\beta_{\text{ConstructionPGCa}}=0.062$), which is plausible because it is the much rarer construction (see Section~\ref{sec:corpusstudies}).
However, if it occurs in a prototypical situation, reading times drop ($\beta_{\text{ConstructionPGCa:Modelprediction}}=-0.132$).
This is noticeable in the downward slope of curve in the right panel of Figure~\ref{fig:spr:effects}.
