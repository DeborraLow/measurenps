...
In a probabilistic framework like Prototype Theory, the crucial question is, however, what controls speakers' decisions to use either variant beyond simple lemma preference effects. 
I argue that the NAC and the PGC are prototypes associated with different degrees of the grammaticalisation of the measure noun, morpho-syntactic properties, and register effects.
...
In (\ref{ex:determiners:nac}), the measure noun is modified by a cardinal \textit{drei} `three', and hence the \NACa\ (the kind NP shows case agreement) is preferred.
In (\ref{ex:determiners:pgc}), the measure noun is modified by a non-cardinal determiner \textit{einige} `some', and the \PGCa\ (the kind NP is in the genitive) is preferred.
...
Instead of inexact Wald approximations or Likelihood Ratio Tests, I used a drop-in bootstrap replacement for the Likelihood Ratio Test from the function \textit{PBmodcomp} from the \textit{pbkrtest} package \citep{HalekohHojsgaard2014}, hence I call the value $p_{\text{PB}}$.
